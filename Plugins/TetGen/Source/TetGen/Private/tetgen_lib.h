...
#define fastlookup(pool, index) \
  (void *) ((pool)->toparray[(index) >> (pool)->log2objectsperblock] + \
	...
	void initializetetgenmesh()
	{
		...
		samples = 0l;
		randomseed = 1l;
		minfaceang = minfacetdihed = PI_tetgen;
		cos_facet_separate_ang_tol = cos(179.9 / 180. * PI_tetgen);
		cos_collinear_ang_tol = cos(179.9 / 180. * PI_tetgen);
		tetprism_vol_sum = 0.0;
